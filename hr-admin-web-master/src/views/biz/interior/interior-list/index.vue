		...
		>
			<template #operator class="table-operator">
				<a-space>
					<a-button
						...
						v-if="hasPerm('bizInteriorAdd') && categoryId && categoryId !== ''"
					>
						<template #icon><plus-outlined /></template>
						新增
					</a-button>
					<xn-batch-button
						v-if="hasPerm('bizInteriorBatchDelete')"
						buttonName="批量删除"
						icon="DeleteOutlined"
						:selectedRowKeys="selectedRowKeys"
						@batchCallBack="deleteBatchbizInterior"
					/>
				</a-space>
			</template>
			<template #bodyCell="{ column, record }">
				...
				<template v-if="column.dataIndex === 'action'">
					<a-space>
						<a @click="formRef.onOpen(record, selectCategoryId.value)" v-if="hasPerm('bizInteriorEdit')">编辑</a>
						<a-divider type="vertical" v-if="hasPerm(['bizInteriorEdit', 'bizInteriorDelete'], 'and')" />
						<a-popconfirm title="确定要删除吗？" @confirm="deletebizInterior(record)">
							<a-button type="link" danger size="small" v-if="hasPerm('bizInteriorDelete')">删除</a-button>
						</a-popconfirm>
					</a-space>
				</template>
			</template>
...
<script setup name="accessory">
	...
	import Form from '@/views/biz/interior/interior-form/index.vue'
	import bizInteriorApi from '@/api/biz/bizInteriorApi'
	...
	// 操作栏通过权限判断是否显示
	if (hasPerm(['bizInteriorEdit', 'bizInteriorDelete'])) {
		columns.push({
			title: '操作',
			dataIndex: 'action',
			align: 'center',
	...
	const editStatus = (record) => {
		...
		bizInteriorApi
			.bizInteriorSubmitForm(formDataParam, true)
			.then(() => {
	...
	const loadData = (parameter) => {
		...
		return bizInteriorApi.bizInteriorPage(Object.assign(parameter, searchFormParam)).then((data) => {
			return data
		})
	}
	// 从封面图URL中提取第一张图片
	...
	// 删除
	const deletebizInterior = (record) => {
		let params = [
			{
				id: record.id
			}
		]
		bizInteriorApi.bizInteriorDelete(params).then(() => {
			tableRef.value.refresh(true)
		})
	}
	// 批量删除
	const deleteBatchbizInterior = (params) => {
		bizInteriorApi.bizInteriorDelete(params).then(() => {
